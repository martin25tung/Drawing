package com.example.drawing.drag_nestedscroll.nested_scroll;
...
public class ScalableImageView extends View implements NestedScrollingChild2 {
  ...
  @Override public boolean startNestedScroll(int axes, int type) {
    return childHelper.startNestedScroll(axes, type); // 默認實現都給 childHelper 做
  }

  @Override public void stopNestedScroll(int type) {
    childHelper.stopNestedScroll(type); // 默認實現都給 childHelper 做
  }

  @Override public boolean hasNestedScrollingParent(int type) {
    return childHelper.hasNestedScrollingParent(type); // 默認實現都給 childHelper 做
  }

  @Override public boolean dispatchNestedScroll(int dxConsumed, int dyConsumed, int dxUnconsumed,
      int dyUnconsumed, @Nullable int[] offsetInWindow, int type) { // 默認實現都給 childHelper 做
    return childHelper.dispatchNestedScroll(dxConsumed, dyConsumed, dxUnconsumed, dyUnconsumed, offsetInWindow, type);
  }

  @Override public boolean dispatchNestedPreScroll(int dx, int dy, @Nullable int[] consumed,
      @Nullable int[] offsetInWindow, int type) { // 默認實現都給 childHelper 做
